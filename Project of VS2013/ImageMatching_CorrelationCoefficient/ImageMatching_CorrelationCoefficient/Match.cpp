...
void CMatch::FeatureMatchMain(Mat LeftImg, Mat RightImg, Mat LeftImgColor, Mat RightImgColor)
{
	...
	//搜索匹配点  
	for (int i = halfsize; i<Lr - halfsize; i++)
	{
		for (int j = 20; j<Lc - halfsize; j++)
		{
			if (Interest.at<uchar>(i, j) == 0)
				//特征点作为模板中心  
			{
				double maxscore = 0;
				for (int r = i+170-20 ; r < i+170+ 20; r++){
					for (int c = j-20 ; c< j+ 20; c++)
					{
				...
				for (int r = i+170-20; r < i+170+20; r++){
					for (int c = j- 20; c < Rr +20; c++)
					{
						double score = NCCScore(i, j, r, c);
						if ((score == maxscore) && (score>Threshold))
						{
							//用直线连接同名点  
							line(Result, Point(j, i), Point(c + RightImgEpi.cols, r), cvScalar(0, 0, 255));
							//将匹配结果存入数组  
